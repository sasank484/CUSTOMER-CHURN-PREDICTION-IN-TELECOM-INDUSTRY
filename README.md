...
In competitive telecom markets, customers can switch providers easily, turning churn into a major revenue risk. This project builds a scalable machine learning pipeline that predicts which customers are likely to leave, so retention teams can act early. Unlike single-model approaches (e.g., a lone Decision Tree or Random Forest), we combine Big Data platforms (Databricks, MongoDB) with robust feature selection and ensemble modeling to handle large, high-dimensional datasets efficiently. The result is faster training, clearer insights, and more accurate churn risk scores that prioritize the most actionable customer segments.
...
4.	Enable Docker using the commands “sudo systemctl start docker” and “sudo systemctl enable docker”
...
7.	Import the data into MongoDB using the command “docker exec -i my-mongo-container mongoimport --db my_database --collection my_collection --type csv --file /data/telecom_churn.csv --headerline”. In this command, “mongoimport” is the function used to import the data, “my_database” is the database name, and “my_collection” is the collection name, since the data in MongoDB is saved as documents in a collection. We have mentioned the file path as “/data/telecom_churn.csv”.
 ![image](https://github.com/user-attachments/assets/39fab56d-a749-4644-b926-e6ac90a2b840)

🌍Setting up Ngrok:
1.	To use Ngrok, we need to set up a free account and update credit card  information to gain access(There is no charge for up to 1GB transfers). We need to use the command provided in Ngrok along with the token to enable Ngrok.
...
📜Running the IPython notebook file:
Upload the IPython notebook file to Databricks.
